...
air_mat_ID       = 6;
alu_mat_ID       = 7;
detector_mat_ID  = water_mat_ID = 5; % water inside in the MCNP input
pneumatic_mat_ID = air_mat_ID = 6; 
pneu_clad_mat_ID = 9; % small and large pneumatics clads are Aluminium 
source_mat_ID    = 10; % air or water
% densities
...
air_density           = pneumatic_mat_ID; 
alu_density           = -2.7; % strucural aluminium 6061 in g/cc
pneumatic_density     = air_density = 1.0E-05;
...
switch pin_type
  ...
  case 'water'
    ...
    case 'detector'
        ...
        surf_ID=surf_ID+1; % increment surface ID
        % write new cylinder surface in file_handle_surf
        fprintf(file_handle_surf,'%5d  RPP %g %g %g %g %g %g \n',surf_ID,x_center-x_block+thickness_clad,x_center+x_block-thickness_clad, y_center-y_block+thickness_clad,y_center+y_block-thickness_clad, -z_block,z_block);
        cell_ID=cell_ID+1; % increment cell ID
        % write new cell in file_handle_cell
    ...
    case 'source'
        ...
        surf_ID=surf_ID+1; % increment surface ID
        % write new cylinder surface in file_handle_surf
        fprintf(file_handle_surf,'%5d  RPP %g %g %g %g %g %g \n',surf_ID,x_center-x_block+thickness_box_x,x_center+x_block-thickness_box_x, y_center-y_block+thickness_box_y,y_center+y_block-thickness_box_y, -z_block,z_block);
        cell_ID=cell_ID+1; % increment cell ID
        % write new cell in file_handle_cell
        fprintf(file_handle_cell,'%5g  %d %g %d %d %d %d imp:n=1 \n',cell_ID,graphite_mat_ID,graphite_density,-surf_ID,+surf_ID-1,z_min,-z_max);
